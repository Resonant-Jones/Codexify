...
# Optional VaultNode Soft Serve forge. This is deliberately separate from the
# supported Codexify Compose graph and never edits Git remotes.
soft-serve-config:
	bash scripts/ops/soft_serve.sh config

soft-serve-up:
	bash scripts/ops/soft_serve.sh up

soft-serve-down:
	bash scripts/ops/soft_serve.sh down

soft-serve-status:
	bash scripts/ops/soft_serve.sh status

soft-serve-logs:
	bash scripts/ops/soft_serve.sh logs
...
canonical-audit-live-proof-receipt:
	@test -n "$(machine_id)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(machine_role)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(authority_basis)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(compose_file)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(compose_project)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(project_role)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(profile_name)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(audit_project)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(api_base)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	@test -n "$(frontend_base)" || { echo "Usage: make canonical-audit-live-proof-receipt machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml compose_project=... project_role=serving|audit profile_name=... audit_project=... api_base=http://127.0.0.1:8888 frontend_base=http://localhost:5173" >&2; exit 2; }
	$(PYTHON) scripts/audit/collect_canonical_live_proof_receipt.py $(if $(repo),--repo "$(repo)",) --machine-id "$(machine_id)" --machine-role "$(machine_role)" --authority-basis "$(authority_basis)" $(if $(assert_canonical_machine),--assert-canonical-machine,) --compose-file "$(compose_file)" --compose-project "$(compose_project)" --project-role "$(project_role)" --profile-name "$(profile_name)" --audit-project "$(audit_project)" --api-base "$(api_base)" --frontend-base "$(frontend_base)" $(if $(serving_project),--serving-project "$(serving_project)",) $(if $(env_file),--env-file "$(env_file)",) $(if $(profiles_dir),--profiles-dir "$(profiles_dir)",) $(if $(migration_dir),--migration-dir "$(migration_dir)",) $(if $(command_timeout),--command-timeout "$(command_timeout)",) $(if $(http_timeout),--http-timeout "$(http_timeout)",) $(if $(output),--output "$(output)",) $(if $(replace),--replace,)

canonical-audit-evidence-generate:
	@test -n "$(metadata)" || { echo "Usage: make canonical-audit-evidence-generate metadata=path/to/input.json machine_id=... machine_role=... authority_basis=... compose_file=docker-compose.yml audit_project=..." >&2; exit 2; }
	@test -n "$(machine_id)" || { echo "Usage: make canonical-audit-evidence-generate metadata=... machine_id=... machine_role=... authority_basis=... compose_file=... audit_project=..." >&2; exit 2; }
	@test -n "$(machine_role)" || { echo "Usage: make canonical-audit-evidence-generate metadata=... machine_id=... machine_role=... authority_basis=... compose_file=... audit_project=..." >&2; exit 2; }
	@test -n "$(authority_basis)" || { echo "Usage: make canonical-audit-evidence-generate metadata=... machine_id=... machine_role=... authority_basis=... compose_file=... audit_project=..." >&2; exit 2; }
