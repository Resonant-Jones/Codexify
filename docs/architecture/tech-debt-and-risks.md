...
- Risk statement: Completion assembly, provider execution, bounded tool-loop reinjection, persistence, and event emission still converge in a small high-coupling area. | Subsystem tag: `chat/core-loop` | Severity: high | Evidence: `guardian/core/chat_completion_service.py`, `guardian/workers/chat_worker.py` | Suggested mitigation: split the completion pipeline into typed stages with focused contract tests, and keep the bounded tool turn isolated from any future multi-turn planner work.
...
- Risk statement: The runtime `/api/tools` shim and `ToolJob` ORM model have been removed, but upgraded databases may still carry the historical `tool_jobs` table from the old migration. No supported path reads or writes that table now; drop it in a follow-up migration when schema cleanup is explicitly approved. | Subsystem tag: `tools` | Severity: low | Evidence: `guardian/db/migrations/versions/9b3d2d08f7c1_add_tool_jobs_table.py`, `docs/architecture/legacy-tools-shim-inventory.md` | Suggested mitigation: add a dedicated schema-drop migration later, after verifying upgrade coverage and any DBs that still need the historical table for rollback continuity.
