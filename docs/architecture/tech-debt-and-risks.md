...
- Risk statement: The runtime `/api/tools` shim has been removed, but the `tool_jobs` schema artifact remains in Postgres as deferred cleanup. No supported path reads or writes that table now; drop it in a follow-up migration so schema truth matches runtime truth. | Subsystem tag: `tools` | Severity: low | Evidence: `guardian/db/models.py:941` (ToolJob model), `guardian/db/migrations/versions/9b3d2d08f7c1_add_tool_jobs_table.py`, `docs/architecture/legacy-tools-shim-inventory.md` | Suggested mitigation: remove the orphaned `tool_jobs` model/migration in a dedicated cleanup task after verifying no remaining consumers.
