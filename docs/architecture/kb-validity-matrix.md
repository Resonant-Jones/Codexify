...
| `/docs/architecture/browser-host-guardian-contract.md` | Browser Host/Guardian protocol, Browser Context Envelope, attachment-grant contract, and development HTTP adapter | `supplementary_verify_against_code` | no | no | Grant schemas, canonical tokens, shared fixtures, the pure Guardian seam, the explicitly gated development HTTP adapter, and the development Browser Host client integration are test-proven; production authenticated issuance, durable persistence, and supported release remain unproven. | Use for implementation and conformance work; retain `supplementary_verify_against_code`; unsafe for current supported runtime diagrams and release claims. |
