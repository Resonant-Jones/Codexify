...
| `/docs/architecture/browser-host-guardian-contract.md` | Browser Host/Guardian protocol and Browser Context Envelope | `supplementary_verify_against_code` | no | no | Normative contract package and schemas exist, but live Browser Host and Guardian integration remain unproven. | Use for implementation and conformance work; verify product claims through live supported-path proof. |
