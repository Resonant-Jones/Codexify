...
| `/docs/architecture/browser-authority-and-context-boundary-contract.md` | browser authority / context provenance | `supplementary_verify_against_code` | no | no | Normative authority, capture, provenance, retention, and failure contract; it does not prove page capture, a Browser Host, browser actions, or current runtime topology. | Use for browser planning and architecture review, then verify implementation and live proof before making runtime or product claims. |
| `/docs/architecture/browser-host-comparative-proof-harness-spec.md` | browser host proof methodology | `supplementary_verify_against_code` | no | no | Normative comparative proof methodology for future Browser Host candidates; not current implementation, topology, or release proof. | Use to author and review common candidate proofs, then verify every candidate through live evidence before ADR or release claims. |
| `/docs/architecture/browser-host-guardian-contract.md` | Browser Host/Guardian protocol, Browser Context Envelope, attachment-grant contract, and development HTTP adapters | `supplementary_verify_against_code` | no | no | Grant schemas, canonical tokens, shared fixtures, the pure Guardian seam, independently gated development negotiation and attachment adapters, and client unit/configuration paths are test-proven. Post-restart, the Apple control passes; repository and clean locked Electron `43.2.0` bundles have equal executable and resource-manifest hashes, both fail the same signature/Gatekeeper resource validation, and both minimal applications launch. The comparison is `clean_electron_43_2_0_also_fails`; production Browser Host and full real-Guardian process qualification, durable persistence, and supported release remain `next-proof-needed` or unproven. | Use for implementation and conformance work; verify the post-restart packet against code; retain `supplementary_verify_against_code`; unsafe for current supported runtime diagrams and release claims. |
| `/docs/architecture/adr/054-browser-host-topology-and-release-ownership.md` | Browser Host topology and release ownership | `supplementary_verify_against_code` | no | no | Accepted future architecture for the Electron/bundled-Chromium Browser Host and monorepo-first ownership boundary; implementation and release posture remain unproven. | Use for Browser Host implementation and ownership decisions; verify product claims against code and supported-path proof. |
