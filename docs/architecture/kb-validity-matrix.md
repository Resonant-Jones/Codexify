...
| `/docs/architecture/browser-host-guardian-contract.md` | Browser Host/Guardian protocol, Browser Context Envelope, and attachment-grant contract | `supplementary_verify_against_code` | no | no | Grant schemas, canonical tokens, shared fixtures, and a pure Guardian validation/one-use seam are now test-proven; live Guardian routing and authenticated issuance remain unproven. | Use for implementation and conformance work; retain `supplementary_verify_against_code`; unsafe for current supported runtime diagrams and release claims. |
