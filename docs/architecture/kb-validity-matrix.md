...
| `/docs/architecture/browser-host-guardian-contract.md` | Browser Host/Guardian protocol, Browser Context Envelope, attachment-grant contract, and development HTTP adapters | `supplementary_verify_against_code` | no | no | Grant schemas, canonical tokens, shared fixtures, the pure Guardian seam, independently gated development negotiation and attachment adapters, and client unit/configuration paths are test-proven; the live Electron launch packet is `next-proof-needed` at the pinned Electron process boundary, and the follow-on macOS qualification classifies matching Apple-control/current/clean Code Signing subsystem errors as `host_code_signing_subsystem_unavailable`, while full real-Guardian process qualification, durable persistence, and supported release remain unproven. | Use for implementation and conformance work; verify the live packet against code; retain `supplementary_verify_against_code`; unsafe for current supported runtime diagrams and release claims. |
