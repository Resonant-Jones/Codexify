...
| `/docs/architecture/browser-host-guardian-contract.md` | Browser Host/Guardian protocol and Browser Context Envelope | `supplementary_verify_against_code` | no | no | A bounded production one-tab skeleton and deterministic negotiation proof now exist, but live production Guardian integration remains unproven. | Use for implementation and conformance work; retain `supplementary_verify_against_code` and verify supported-product claims against code and live proof. |
