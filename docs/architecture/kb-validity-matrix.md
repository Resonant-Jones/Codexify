...
| `/docs/iddb_policy_v1.md` | identity data policy (canonical path) | `design_canon_not_runtime_truth` | no | no | IDDB policy v1 covering diary/identity layer separation, Imprint_Zero/light identity, opt-in deep identity, persona borrowing semantics, and sensitive-trait non-inference rules. Both this canonical path and `/docs/guardian/iddb_policy_v1.md` are valid; prefer this path for audit tooling resolution. | Use for identity/governance interpretation; not a first-pass runtime topology source. |
