...
70. [[071-connections-control-plane-boundary|ADR-071 Connections Control Plane Boundary]] — Accepted read-only catalog and projection boundary for the Settings Connections bay; catalog visibility, implementation, setup, authorization, and health remain distinct, and existing subsystem mutation APIs retain authority.
71. [[072-bounded-settings-and-connections-route-promotion|ADR-072 Bounded Settings and Connections Route Promotion]] — Accepted 2026-08-18 by Resonant Jones. Promotes the local profile's authenticated identity/prompt settings surfaces and the read-only Connections catalog while keeping generic connector execution quarantined and publishing mounted route truth through `/health`.
