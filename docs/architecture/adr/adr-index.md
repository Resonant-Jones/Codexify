...
44. [[053-node-hosted-room-access-boundary|ADR-053 Node-Hosted Room Access Boundary]] — proposed (docs-only) architecture record defining the room-scoped authority, invitation, participant, canonical-thread, network-reachability, lifecycle, and agent-invocation boundaries for future Node-Hosted Rooms. Does not implement Hosted Room runtime behavior or widen release claims.
...
- [[053-node-hosted-room-access-boundary|ADR-053 Node-Hosted Room Access Boundary]]
  - Governs the proposed, docs-only boundary for future node-owned Hosted Rooms: one canonical backing thread, separate network reachability and room authority, room-scoped invitations and sessions, explicit participants and agents, bounded presence, and fail-closed lifecycle behavior.
