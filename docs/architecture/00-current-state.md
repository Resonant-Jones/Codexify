...
- [ADR-067 private-preview Chroma topology](./adr/067-operator-approved-derived-chroma-retirement.md) is frozen as a bounded decision: one explicit Docker-managed local named volume, `codexify_private_preview_chroma`, external to Compose lifecycle and shared at `/app/.chroma`. The diagnosed host bind is not approved for private-preview recovery. Compose/config implementation, live retirement/fresh initialization, matching Persona deployment, and authenticated route/browser save/readback remain pending. Historical preservation remains separate; periodic reconciliation remains suspended. This is documentation-only and changes no Beta/release claim.

- [Persona SQLite host-storage diagnosis](./proofs/runtime/2026-09-07-persona-private-preview-sqlite-host-storage-diagnosis.md) classifies `DOCKER_HOST_BIND_SQLITE_IDENTITY_DEFECT`: the exact runtime fails the first SQLite insert with `SQLITE_READONLY_DBMOVED` across the worktree bind, an outside-worktree bind on the same APFS mount, and a separate APFS mount. Prior named-volume/internal controls succeed. The storage-topology decision is now frozen, but ADR-067 recovery remains blocked pending its separate implementation and ordered live proof; no retirement, runtime recovery, or Persona qualification is claimed.
