...
- Added the internal authenticated `GET /api/system_prompt/inspect` backend signal: canonical thread profile ID/revision observation, active Imprint metadata, independent system-document counts, and legacy-free inspection-builder measurements with per-layer failure states. Focused tests cover scope isolation, exact/revisionless/absent/unavailable selection, no legacy Persona reads, safe metadata, and compatibility endpoints. Frontend migration remains the next prerequisite; this is not supported-profile exposure, live-browser qualification, or release support.

- Removed frontend clients and controls for the retired Persona mutation endpoint: Settings no longer edits or syncs legacy Persona prompt text, and obsolete Persona panels/hooks are deleted. Unrelated local preview fields and Imprint proposal/accept/reject remain unchanged; retained status consumers observe legacy Persona state read-only. No replacement API or local Persona authority was introduced. Legacy Persona storage/status observation and canonical Persona Studio adoption remain unfinished. The next seam is frontend Inspector migration to the canonical read-only signal; no release/support claim widens.
...
- Accepted ADR-058 separating canonical Persona Profile authored authority from Imprint relational/presentation ownership; legacy Persona observation/status, canonical Persona Studio adoption, and inspector convergence remain unfinished, and no Beta/support claim changed.
