...
- Do not assume cross-node private messaging, node resolution/trust, Guardian messaging, DM attachments, or a frontend Inbox: all remain unproven and explicitly deferred by ADR-077. Do not assume Project invitation acceptance, Guardian Conversation retrieval, retrieval/disclosure scope policy, ambient DM context, Share Sheet UX, or agent-authored peer messages: all remain unproven and explicitly deferred by ADR-078. The direct-messaging surface must not be treated as Beta-supported on the default profile.
