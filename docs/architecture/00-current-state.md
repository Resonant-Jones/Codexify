...
On 2026-05-09, after syncing `codex/refresh-currentstate-truth` to `origin/main` at `bc2b672d2` (branch now at `ab74fb174` which includes it), live supported-path recheck confirmed upload -> embed -> readback: document upload returned explicit `document_id` + `media_asset_id`, `GET /api/documents/{id}` returned `200`, embedding reached `ready`. Sentinel retrieval through chat surface not tested in this run (worker-chat not started). The `document_id` field and `GET /api/documents/{id}` route confirm the document identity/readback contract is repaired and the document is accessible after embedding.
...
- Upload -> parse -> embed -> retrieve is PARTIALLY proven on the live runtime (2026-05-09 recheck): upload succeeds with explicit `document_id` + `media_asset_id`; `GET /api/documents/{id}` returns `200`; embedding reaches `ready`; sentinel retrieval through chat surface not tested (worker-chat not started). The earlier `agent_extension_*` schema gap that blocked document upload on the supported path is repaired.
