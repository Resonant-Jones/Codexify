...
`main` is in local-first beta hardening at `f7a11746d`. The current release interpretation remains the local Docker Compose path and local-only provider posture. Recent DLG/PAO, authentication, chat tool-boundary, and Coding Loop work is merged code, evidence, or contract/test surface; it does not widen the release promise without matching supported-path runtime proof.

## What changed recently

- Added DLG Phase 3A validation/projections and four fixed Phase 3B Agent Reading Packet fixtures; this is deterministic control-plane calibration, not runtime retrieval.
- Added a Phase 4A ADR-number collision review; recommendations remain advisory and pending human canonicalization decisions.
- Merged linked-email login fallback with its migration and runtime-configuration regression coverage; this does not widen the beta surface.
- Added focused chat tool-advertisement boundary coverage; ordinary chat passes no effective tools, while unadvertised-command rejection remains an explicit strict xfail.
- Merged the bounded Coding Loop profile/guard work and a live-proof packet; the packet stopped at backend startup, before route acceptance, execution, or durable readback.
- Added generated DLG/PAO artifacts and proof receipts; these are repository/control-plane evidence, not live-service readiness proof.
...
- Do not assume chat tool-boundary tests or the Coding Loop proof packet establish enforcement, provider execution, terminal persistence, or durable source-thread readback.
...
- Fresh live Compose proof is still needed at `f7a11746d`, including health, model inventory, terminal completion, persisted output, and retrieval.
...
1. Capture fresh supported-Compose proof at `f7a11746d`: health, model inventory, chat, persistence, and retrieval.
...
4. Re-prove the enabled Coding Loop lane end to end, or quarantine it until backend, adapter, terminal, and durable readback proof exists.
