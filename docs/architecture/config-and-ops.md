...
| `LOCAL_BASE_URL`, `LOCAL_DOCKER_FALLBACK_BASE_URL`, `LOCAL_API_KEY`, `LOCAL_CHAT_MODEL`, `LOCAL_PROVIDER_DISPLAY_NAME`, `LOCAL_PROVIDER_VENDOR`, `LOCAL_COMPAT_FIRST`, `LOCAL_EMBED_MODEL` | Local provider connectivity and model selection; current defaults target Whoosh'd through an OpenAI-compatible `http://host.docker.internal:8000/v1` gateway, with Docker host bridge fallback when loopback is configured | `guardian/core/config.py`, `guardian/core/ai_router.py`, `docker-compose.yml` |
...
- The wizard/launcher presents the user-facing local provider posture as “Local via Whoosh'd.”
- Whoosh'd is a host-local inference node, not code vendored into Codexify. Use [`../Ops/WHOOSHD_LOCAL_RUNTIME_RUNBOOK.md`](../Ops/WHOOSHD_LOCAL_RUNTIME_RUNBOOK.md) to install, start, and prove the local Whoosh'd runtime before using the supported Compose path.
- The machine config remains split across the legacy and canonical lanes: `AI_BACKEND=ollama` plus `LLM_PROVIDER=local`, with `LOCAL_BASE_URL=http://host.docker.internal:8000/v1` for the Docker Compose runtime.
