...
If you are working on human-agent authorship boundaries, delegated authority, review/approval state, or provenance footers for external communication, start with [`Agency Provenance Layer`](./agency-provenance-layer.md). That spec is a Draft umbrella over identity, delegation, and lineage; it is docs-only, does not widen the release promise, and points back to [`00-current-state.md`](./00-current-state.md) for release truth.
...
- [Agency Provenance Layer](./agency-provenance-layer.md): Draft umbrella spec for human-agent authorship, delegated authority, review state, and auditability across product surfaces; docs-only candidate tokens, not runtime support, and does not override `00-current-state.md`.
