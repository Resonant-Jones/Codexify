...
- [Continuity Operator Loop Proof Chain](./continuity-operator-loop-proof-chain.md): docs-only consolidation of the Continuity operator write/readback/diagnostics proof chain. Summarizes 15 proven artifacts, operator triad properties, gate model, supported beta quarantine, hard-false flags, no-ambient invocation boundary, data exposure boundary, release claim boundary, and safe next task options. Does not implement routes, activate profiles, or widen the release promise.
