...
- [Continuity Operator Loop Proof Chain](./continuity-operator-loop-proof-chain.md): docs-only consolidation of the full six-route Continuity operator proof chain. Summarizes write, packet readback, diagnostics, state readback, commit readback, and link readback. Covers test-only profile gates, supported beta quarantine, feature flag, API-key auth, missing-record behavior, no-write readback, hard-false flags, data exposure boundary, staged readback completion, and release claim boundaries. Does not implement routes, activate profiles, or widen the release promise.
