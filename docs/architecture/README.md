...
- [Zac Collaborator RAG Source Directory](../collaborators/zac/README.md): collaborator RAG source directory for Zac and Zac's agent; supports inspiration-led exploration and proposal-before-change workflow; includes a copy-paste agent startup prompt, agent brief, proposal protocol, safe/sensitive zones, proposal template, and source map; expected first agent output is a scout report/proposal, not implementation; does not replace architecture docs or `00-current-state.md`; does not authorize runtime semantic changes without the proper Codexify task lane.
