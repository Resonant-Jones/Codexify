...
- [Flow Builder TestRun Harness](./flow-builder-testrun-harness.md): in-memory non-side-effecting TestRun proof harness for Flow Builder; validates FlowDraft against supported subset, simulates deterministic execution without model calls or external writes, and produces RunReceipt-shaped proof. FB-012 proof artifact. Not API support, not persistence, not runtime execution.
