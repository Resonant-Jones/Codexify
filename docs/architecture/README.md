...
Read it first for live release readiness, supported install path, active blockers, and short-horizon priorities.
...
- [Flow Builder TestRun Harness](./flow-builder-testrun-harness.md): in-memory non-side-effecting TestRun proof harness for Flow Builder plus FB-013 gated in-memory side-effect proof path; validates FlowDraft against supported subset, simulates deterministic execution without model calls or external writes, and records receipt-shaped side-effect evidence only when explicitly gated. FB-012/FB-013 proof artifact. Not API support, not persistence, not runtime execution.
