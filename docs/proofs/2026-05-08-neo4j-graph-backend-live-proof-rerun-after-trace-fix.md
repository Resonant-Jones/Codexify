...
1. **Chat completion stalled at `AWAITING_FIRST_TOKEN`**: The Ollama inference endpoint (`http://100.109.4.57:11434`) did not return a first token within the observed window (~90 seconds). This is a separate runtime/llm-inference issue. The task reached `AWAITING_FIRST_TOKEN` state but did not complete.
