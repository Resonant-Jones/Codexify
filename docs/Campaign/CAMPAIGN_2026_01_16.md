...
Edit only the files that implement and invoke the Create Project modal:
 • frontend/src/components/sidebar/SidebarRoot.tsx
 • frontend/src/components/projects/CreateProjectModal.tsx (or the actual modal component file used in the sidebar)
 • frontend/src/hooks/useProjects.ts (or the hook/service used to create/list projects)

If the modal component/hook lives at a different path than listed above, stop and report the discovered path before editing anything else.
