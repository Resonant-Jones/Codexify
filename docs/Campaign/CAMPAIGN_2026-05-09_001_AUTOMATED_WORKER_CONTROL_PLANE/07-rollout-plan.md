...
- Supported coding-execution note (2026-05-25): the public coding route should use the Pi broker adapter lane (`pi`, `pi_sdk`, `pi_codex_runner`). Direct `codex` and `claudecode` adapter kinds are removed/unsupported for this module, and the local Compose worker keeps `./codex_runner` mounted at `/app/codex_runner` so the Pi runner artifact is present when that path is selected.
- Historical proof note (superseded): an earlier 2026-05-13 local smoke used `adapter_kind="codex"` before the direct-adapter migration. That note is retained only as historical context and is not current runtime doctrine.
