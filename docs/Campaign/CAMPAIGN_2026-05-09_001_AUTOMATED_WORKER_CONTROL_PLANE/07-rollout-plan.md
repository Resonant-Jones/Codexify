...
- Supported coding-execution note (2026-05-26): the public coding route now treats Pi as the direct Campaign Runner broker adapter seam. Public requests should use `adapter_kind="pi"`, while the worker preserves `pi_codex_runner` only as a legacy-compatible internal target name. Direct `codex` and `claudecode` adapter kinds are unsupported and fail closed. The local Compose worker still mounts `./codex_runner` at `/app/codex_runner` so the Pi runner artifact is present when that path is selected.
- Supported coding-execution proof note (2026-05-26): future live proof for this module must demonstrate Pi-brokered execution with stored backend receipts that disclose the resolved downstream provider/model identity. Prior direct-Codex proof notes are historical only and do not describe the current supported adapter contract.
