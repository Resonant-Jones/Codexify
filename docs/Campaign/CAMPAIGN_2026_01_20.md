...
Goal: Add E2E coverage for the ChatGPT migration flow. The test should validate that uploading a ChatGPT export triggers the canonical endpoint and results in a UI refresh (threads list updates) without manual reload.

Constraints / Requirements
- Prefer Playwright and the existing test harness patterns in this repo.
...
- Must validate at minimum:
  ...
  - The import request hits POST `/api/upload-chatgpt-export`.
  - The success path triggers a threads refresh signal OR results in a visible thread list update.
