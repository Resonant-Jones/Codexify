...
- task_artifact_path: docs/work/tasks/<RUNNER_DETERMINES>.md
- allowed_files:
  - docs/work/reports/security/oauth_rotation_receipt.md
- command_checklist:
  1. Preflight: git status --porcelain -uall must be empty
  ...
  5. mkdir -p docs/work/reports/security
  6. printf "# OAuth Rotation Receipt\n- Rotated At (UTC): %s\n- Change Ticket: %s\n- Scope: guardian/secrets/client_secret_oauth.json and guardian/secrets/token.json\n- Operator Confirmation: Provider-side revoke completed for previously exposed OAuth client and tokens.\n" "$GOOGLE_OAUTH_ROTATED_AT_UTC" "$GOOGLE_OAUTH_ROTATION_TICKET" > docs/work/reports/security/oauth_rotation_receipt.md
  7. violations="$(git diff --name-only | rg -v '^(docs/work/reports/security/oauth_rotation_receipt\\.md)$' || true)"; test -z "$violations" || { echo "STOP: out-of-scope files detected"; printf '%s\n' "$violations"; echo "Cleanup: git restore --staged $violations && git restore $violations"; exit 1; }
  8. rg -n "Rotated At|Change Ticket|Provider-side revoke completed" docs/work/reports/security/oauth_rotation_receipt.md
- expected_outputs:
  ...
  - no out-of-scope files are modified
- rollback_commands:
  - git restore docs/work/reports/security/oauth_rotation_receipt.md
- dependencies:
