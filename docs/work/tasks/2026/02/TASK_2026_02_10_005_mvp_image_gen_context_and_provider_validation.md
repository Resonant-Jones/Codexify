...
- Summary: Backend completion response is now explicit (task/thread ids plus `messages_url` and `trace_url`), and the frontend fetches the latest RAG trace deterministically after assistant messages instead of depending on `response.data.context`. See `guardian/routes/chat.py:870` and `frontend/src/features/chat/GuardianChat.tsx:18,189,348-444,656-668`. The contract is documented for future implementers in `docs/reference/architecture/completion_pipeline.md:48-67`.
