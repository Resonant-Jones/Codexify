...
- Tests ran: git status --porcelain -uall (pre/post checks, clean preflight and in-scope post-change), git status --porcelain -uall | awk '{print $2}' | grep -Ev '^(frontend/src/hooks/useUploader.ts|frontend/src/lib/api.ts|frontend/src/tests/uploader_document_auth.spec.ts)$' (no out-of-scope paths), pnpm vitest run src/tests/uploader_document_auth.spec.ts --environment jsdom (PASS), pytest -q tests/routes/test_media_routes.py guardian/tests/test_document_embed_worker.py (PASS)
