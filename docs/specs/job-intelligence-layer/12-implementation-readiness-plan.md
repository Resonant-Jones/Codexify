...
The deterministic assembly helper now exists at `scripts/job_intelligence/assemble_fixture_draft.py`. It validates the docs-local fixture first, deterministically assembles Job Profile draft and review packet artifacts from the synthetic extraction fixture, compares generated artifacts against the docs-local expected artifacts, does not call a model, does not create runtime behavior, does not validate extraction quality, does not prove production readiness, and keeps runtime, prompt, schema, persistence, review UI, transcription, consent, retention, pricing, and dispatch deferred.
