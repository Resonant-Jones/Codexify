...
- `Verified:` The current documented chat path is `POST /api/chat/{thread_id}/complete` -> Redis enqueue -> worker execution -> context/prompt assembly -> provider call -> assistant-message persistence -> task-event emission. (`docs/reference/architecture/system-overview.md`, `docs/reference/architecture/completion_pipeline.md`, `docs/reference/architecture/flows.md`)
- `Verified:` The completion path is queue-coupled and worker-driven, with per-thread turn gating and Redis task-event transport called out as part of the implemented runtime rather than as speculation. (`docs/reference/architecture/system-overview.md`, `docs/reference/architecture/completion_pipeline.md`, `docs/reference/architecture/data-and-storage.md`)
...
- `Verified:` The requested runtime docs describe a direct tools execution path that is immediate and process-local, separate from durable cron execution. (`docs/reference/architecture/system-overview.md`, `docs/reference/architecture/modules-and-ownership.md`, `docs/reference/architecture/flows.md`, `docs/reference/architecture/roadmap-signals.md`)
- `Verified:` The current planning docs also explicitly warn that some execution surfaces remain process-local or non-durable. (`docs/reference/architecture/roadmap-signals.md`)
...
- `Verified:` The requested docs describe a cron path in which job definitions live in Postgres, scheduler ticks create durable `cron_runs`, Redis carries execution payloads, and a cron worker records terminal status. (`docs/reference/architecture/system-overview.md`, `docs/reference/architecture/data-and-storage.md`, `docs/reference/architecture/flows.md`)
...
- `Unverified:` Continuous cron execution as an always-on default Compose runtime was not confirmed from inspected runtime wiring; `docker-compose.yml` showed chat, voice, warmup, and embedding workers, but not dedicated cron scheduler/worker services. (`docker-compose.yml`, `docs/reference/architecture/system-overview.md`)

### Task Events, Event Feeds, SSE, and WebSocket Surfaces

- `Verified:` Codexify exposes multiple transport surfaces: durable outbox SSE at `/api/events`, Redis task-event SSE at `/api/tasks/{task_id}/events`, sync SSE, and websocket RPC. The requested docs present these as separate subsystems rather than a single universal communication mechanism. (`README.md`, `docs/reference/architecture/system-overview.md`, `docs/reference/architecture/modules-and-ownership.md`, `docs/reference/architecture/data-and-storage.md`)
...
- `Inference:` Agent-orchestration APIs and CLI adapters should be treated as adjacent seams, not proof of a completed chat-driven delegation system, because current docs describe plans/deployments/runs and event streaming but also explicitly note that real delegated execution may still be scaffolded rather than fully wired. (`docs/reference/guardian/agent-orchestration.md`, `docs/reference/guardian/agent-runtime-onboarding.md`, `guardian/routes/agent_orchestration.py`)
...
- `Reuse current seams`
- `Verified:` Reuse the existing thread/message/project/persona/imprint/system-prompt context model as the planning surface inside Guardian. (`docs/reference/architecture/completion_pipeline.md`, `docs/iddb_policy_v1.md`)
...
- `Reuse current seams`
- `Verified:` Existing runtime already has outbox SSE and task-event SSE surfaces suitable for live progress updates. (`README.md`, `docs/reference/architecture/data-and-storage.md`)
...
- `Verified:` Context assembly can draw from messages, semantic search, memory, graph, sensors, and optional federated context depending on depth. (`docs/reference/architecture/completion_pipeline.md`)
...
- `Verified:` Redis queue/event data is operationally critical but non-durable by default in Compose. (`docs/reference/architecture/data-and-storage.md`, `docker-compose.yml`)
...
- `Verified:` Direct tools are process-local while cron runs are durable. (`docs/reference/architecture/system-overview.md`, `docs/reference/architecture/roadmap-signals.md`)
...
- `Verified:` The runtime already has multiple event surfaces. (`README.md`, `docs/reference/architecture/system-overview.md`)
...
- `docs/reference/architecture/guardian-agent-delegation-recon.md`, `docs/reference/guardian/agent-orchestration.md`, `docs/reference/guardian/agent-runtime-onboarding.md`
- `Blast radius`
