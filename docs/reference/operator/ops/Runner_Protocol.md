...
- **tests before commit**
- **prompt + summary artifacts** recorded in `docs/work/tasks/YYYY/MM/`
...
- The runner MUST use an underscore-named task artifact under `docs/work/tasks/YYYY/MM/`:
  - `docs/work/tasks/YYYY/MM/TASK_YYYY_MM_DD_NNN_<lowercase_slug>.md`
...
4. **Artifacts are mandatory**
   - Every task produces/updates its `docs/work/tasks/...` file with:
     - Task Prompt
...
8. **Canonical file naming (prevents scope + path ambiguity)**
   - **Task artifacts** MUST use underscores and a lowercase slug:
     - Directory: `docs/work/tasks/YYYY/MM/`
     - Filename pattern: `TASK_YYYY_MM_DD_NNN_<lowercase_slug>.md`
     - Example: `docs/work/tasks/2026/01/TASK_2026_01_20_001_chat_endpoint_canonicalization.md`
   - **Campaign files** SHOULD be visually distinct and consistent:
     - Directory: `docs/work/campaigns/YYYY/MM/`
     - Filename pattern: `CAMPAIGN_YYYY_MM_DD.md` (uppercase prefix)
     - Example: `docs/work/campaigns/2026/01/CAMPAIGN_2026_01_20.md`
...
TASK-ID: <task-id>
TASK-ARTIFACT: <docs/work/tasks/YYYY/MM/TASK_YYYY_MM_DD_NNN_slug.md>
BRANCH: <git-branch>
...
Update the task file under `docs/work/tasks/…` with two sections:
...
- Any campaign/task filename/path mismatch due to dash-vs-underscore naming or directory case (e.g., `docs/campaign` vs `docs/work/campaigns`).
...
In `docs/work/campaigns/2026/01/CAMPAIGN_2026_01_20.md`, in the section `## CAMPAIGN_2026_01_20_004_MVP_LOOP_CLOSURE_DOCUMENT_GENERATION` under `### TASK-2026-01-20-013_DOCUMENT_GEN_MODAL_UI`, update the `Files allowed to edit (only)` list by removing the non-existent sidebar file and adding the correct one:
