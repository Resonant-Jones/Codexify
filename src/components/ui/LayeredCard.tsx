 ...
 *
 * Purpose:
 *  - Reusable primitive that renders two tightly stacked "plates":
 *      1) BACK PLATE — slightly lighter (soft occlusion / bezel)
 *      2) FRONT CARD — inset tile (content layer) with real border and inner lip
 *
 * Design decisions / notes:
 *  - We avoid heavy blur masks on the perimeter to eliminate hairline artifacts on rounded corners.
 *  - Instead we use real borders + inset/outer shadows to get the chunky beveled look.
 *  - The component exposes small theme control via props (tone, bevel, depth, glass) to let appshell/dashboard
 *    compose different surfaces without touching CSS.
 *
 * Usage examples:
 *  <LayeredCard className="w-full h-full" tone="base">...</LayeredCard>
 *  <LayeredCard bevel="chunky" glass>...</LayeredCard>
 *
 * Tokens used (CSS custom props set inline by the component):
 *  --inset-3         : inset for the front card (defaults to 3px)
 *  --bezel-thickness : thickness of the decorative bezel/border (defaults to 3px)
 *  --lip-w           : inner highlight shadow offset (defaults to 2px)
 *  --card-radius     : corner radius for cards (defaults to 16px)
 *
 * Defaults: bevel='chunky', glass=true, liquidBezel=true for a bold, chunky beveled look.
 * Pass bevel='small' or set liquidBezel={false} for lighter chrome.
 */
      ...
      {liquidBezel && (
        <div
          ...
          style={{
            ...
            // Cross‑browser gradient ring using background-clip (no mask-composite)
            background:
            ...
            // ensure the ring sits above the back plate but below the front card
            zIndex: 0,
          }}
        />
      )}
...
// Refractive variant that preserves padding and radius while delegating the
// surface to the WebGL-powered RefractiveGlassCard. Falls back gracefully.
export function LayeredRefractiveCard({
  ...
  return (
    <RefractiveGlassCard wallpaperUrl={wallpaperUrl} className={cn("rounded-2xl", className)} style={style}>
