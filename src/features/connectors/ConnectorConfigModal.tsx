...
export const ConnectorConfigModal: React.FC<Props> = ({ connector, open, onClose, onSave }) => {
  ...
  useEffect(() => {
    ...
    setMessage(null);
    api
      .get<Connector>(`/api/connectors/${connector.id}`)
      .then((res) => {
  ...
  async function handleAuthorizeOrSave() {
    ...
    try {
      if (method === "oauth") {
        ...
        const res = await api.post(`/api/connectors/${connector.id}/authorize`, { redirectUri });
        if (res?.data?.authUrl) {
          ...
          const poll = async () => {
            try {
              const s = await api.get(`/api/connectors/${connector.id}`);
        ...
        const resp = await api.post(`/api/connectors/${connector.id}/config`, { fields });
        if (resp?.data?.ok) {
  ...
  async function handleTest() {
    ...
    setMessage(null);
    try {
      const r = await api.post(`/api/connectors/${connector.id}/test`);
  ...
  async function handleSync() {
    ...
    setMessage(null);
    try {
      const r = await api.post(`/api/connectors/${connector.id}/sync`);
