...
export function useConnectors() {
  ...
  async function fetchConnectors() {
    try {
      setLoading(true);
      const res = await api.get<Connector[]>("/api/connectors");
  ...
  async function authorizeOAuth(id: string) {
    try {
      ...
      const res = await api.post(`/api/connectors/${id}/authorize`, { redirectUri });
      if (res?.data?.authUrl) {
  ...
  async function syncConnector(id: string) {
    try {
      const res = await api.post(`/api/connectors/${id}/sync`);
      return res.data;
    } catch (err) {
