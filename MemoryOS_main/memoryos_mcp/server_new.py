...
def add_memory(
    ...
    try:
        ...
        return {"status": "error", "message": f"Error adding memory: {str(e)}"}
...
def retrieve_memory(
    ...
    try:
        ...
        retrieval_results = memoryos_instance.retriever.retrieve_context(
            user_query=query, user_id=memoryos_instance.user_id
        )
        ...
        result = {
            ...
            "user_profile": (
                user_profile
                if user_profile and user_profile.lower() != "none"
                else "No detailed user profile"
            ),
            ...
            "retrieved_user_knowledge": [
                {"knowledge": k["knowledge"], "timestamp": k["timestamp"]}
                for k in retrieval_results["retrieved_user_knowledge"][:max_results]
            ],
            "retrieved_assistant_knowledge": [
                {"knowledge": k["knowledge"], "timestamp": k["timestamp"]}
                for k in retrieval_results["retrieved_assistant_knowledge"][
                    :max_results
                ]
            ],
        ...
        return {"status": "error", "message": f"Error retrieving memory: {str(e)}"}


@mcp.tool()
def get_user_profile(
    include_knowledge: bool = True, include_assistant_knowledge: bool = False
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if include_knowledge:
            user_knowledge = (
                memoryos_instance.user_long_term_memory.get_user_knowledge()
            )
            result["user_knowledge"] = [
                {"knowledge": item["knowledge"], "timestamp": item["timestamp"]}
...
def main():
    ...
    try:
        ...
        print(
            f"MemoryOS MCP Server 已启动，用户ID: {memoryos_instance.user_id}",
            file=sys.stderr,
        )
