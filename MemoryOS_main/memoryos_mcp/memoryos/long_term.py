...
class LongTermMemory:
    ...
    def _search_knowledge_deque(
        ...
        results.sort(
            key=lambda x: float(
                np.dot(np.array(x["knowledge_embedding"], dtype=np.float32), query_vec)
            ),
            reverse=True,
        )
        return results
    ...
    def store_conversation_summary(self, conversation_id, summary_text):
        ...
        if not hasattr(self, "conversation_summaries"):
            self.conversation_summaries = {}
        self.conversation_summaries[conversation_id] = {
            "summary": summary_text,
            "timestamp": get_timestamp(),
        }
        print(f"LongTermMemory: Stored summary for conversation {conversation_id}.")
        self.save()
