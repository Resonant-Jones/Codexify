...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            GroqEmbedder, "groq", "GROQ_API_KEY", False, marks=pytest.mark.api
        ),
        pytest.param(
            OpenAIEmbedder, "openai", "OPENAI_API_KEY", True, marks=pytest.mark.api
        ),
        pytest.param(
            GeminiEmbedder, "gemini", "GEMINI_API_KEY", False, marks=pytest.mark.api
        ),
    ],
    ids=["local", "groq", "openai", "gemini"],
)
def test_embedders_common_interface(
    embedder_class, dim_key, api_key_env_var, init_with_key
):
    ...
    if api_key_env_var:
        ...
        if not api_key:
            pytest.skip(
                f"Skipping {embedder_class.__name__} test: {api_key_env_var} not set."
            )
    ...
    assert (
        len(embedding) == EXPECTED_DIMS[dim_key]
