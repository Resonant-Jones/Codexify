...
def validate_proof_report(report: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(artifacts, dict):
        ...
        for key in ARTIFACT_KEYS:
            ...
            if not isinstance(value, str) or not value:
                errors.append(f"Proof report artifact {key!r} must be a non-empty string path.")
    ...
    if not isinstance(metadata, dict):
        ...
        if not isinstance(generated_at, str) or not generated_at:
            errors.append("Proof report metadata.generated_at must be a non-empty string.")
