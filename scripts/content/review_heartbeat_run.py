...
def _find_report(date_str: str, report_dir: Path) -> Path:
    ...
    if expected.is_file():
        return expected
    raise FileNotFoundError(f"no heartbeat report found for {date_str} in {report_dir}")
...
def _parse_report(text: str) -> dict[str, Any]:
    ...
    # Title
    title_match = re.search(r"^# Heartbeat Orchestrator — (.+)$", text, re.MULTILINE)
    if title_match:
    ...
    # Run summary table
    table_match = re.search(r"\| Step \| Status \|.*?\n\|[-| ]+\n(.*?)(?=\n\n)", text, re.DOTALL)
    if table_match:
...
def _mark_issue(review: dict[str, Any], message: str, *, strict: bool = False) -> None:
...
def review_run(
    *,
    ...
    if not parsed["run_summary"]:
        _mark_issue(review, "No run summary table found in report", strict=strict)
        return review

    # 3b. Verify report title matches expected format
    if parsed["title"] is None:
        _mark_issue(review, "Report title missing or does not match 'Heartbeat Orchestrator — YYYY-MM-DD'", strict=strict)
    elif parsed["title"] != date_str:
        _mark_issue(review, f"Report title date mismatch: expected {date_str}, got {parsed['title']}", strict=strict)

    # 4. Check for failures in the run summary
    failed_steps = [s for s in parsed["run_summary"] if s["status"] == "failed"]
    if failed_steps:
        _mark_issue(review, f"{len(failed_steps)} step(s) failed: {', '.join(s['step'] for s in failed_steps)}", strict=strict)

    # 4b. In strict mode, non-passed steps (including skipped) are issues
    if strict:
        non_passed = [s for s in parsed["run_summary"] if s["status"] != "passed"]
        if non_passed:
            _mark_issue(review, f"{len(non_passed)} step(s) not passed (strict): {', '.join(s['step'] + ':' + s['status'] for s in non_passed)}", strict=strict)
    ...
    for art in parsed["artifacts"]:
        ...
        if not exists:
            _mark_issue(review, f"Artifact not found on disk: {art}", strict=strict)
    ...
    if secret_hits:
        _mark_issue(review, f"Secret-like values detected in report: {', '.join(sorted(set(secret_hits)))}", strict=strict)
...
def _print_review(review: dict[str, Any]) -> None:
    ...
    for step in run_summary:
        status_mark = "✅" if step["status"] == "passed" else ("❌" if step["status"] == "failed" else "⏭️")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        review = review_run(date_str=date_str, report_dir=args.heartbeat_dir, strict=args.strict)
