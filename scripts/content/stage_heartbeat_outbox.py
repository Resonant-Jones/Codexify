...
def _generate_drafts(
    ...
    for line in summary_lines:
        cells = [c.strip() for c in line.strip("|").split("|")]
        if len(cells) >= 2:
            status_icon = "✅" if "passed" in cells[1] else ("❌" if "failed" in cells[1] else "⏭️")
    ...
    *,
    ...
    for raw in artifact_paths:
        ...
        if dest.exists() and not force:
            result["skipped"].append(f"{art_path.name} (exists, use --force to overwrite)")
            continue
    ...
    manifest = {
        ...
        "review_status": "passed" if result["review_passed"] else ("skipped" if skip_review else "not_passed"),
