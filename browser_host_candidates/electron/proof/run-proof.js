...
async function navigate(trusted, remote, url) {
  ...
  await Promise.all([
    remote.waitForURL(url, { timeout: 30000 }),
    trusted.locator("#navigate").click()
  ]);
  await remote.waitForLoadState("domcontentloaded");
  await sleep(100);
}
...
async function main() {
  ...
  try {
    ...
    await navigate(trusted, promptUrl, remote);
    ...
    await navigate(trusted, formUrl, remote);
    ...
    await navigate(trusted, iframeUrl, remote);
    ...
    await navigate(trusted, originBUrl, remote);
    ...
    await navigate(trusted, raceUrl, remote);
    const race = await capture(trusted, remote, "visible_page_text");
    await remote.locator("#change-identity").click();
    await sleep(100);
    const raceState = await trustedState(trusted);
    record("integrity_document_change_invalidates", raceState.preview === null ? "passed" : "inconclusive", "live-runtime-proven", "Document identity change leaves no attachable preview.");
    ...
    record("integrity_stale_navigation_rejected", raceState.preview === null ? "passed" : "inconclusive", "live-runtime-proven", "Stale capture was invalidated before attachment after document change.");
    await trusted.locator("#attach").click();
    await sleep(100);

    await navigate(trusted, originChangeUrl, remote);
    await capture(trusted, remote, "visible_page_text");
    const targetOriginB = `${runtime.originBUrl}/basic-visible`;
    await navigate(trusted, targetOriginB, remote);
    ...
    record("failure_capture_cancelled", staleState.preview === null ? "passed" : "inconclusive", "live-runtime-proven", "Pending capture is cleared after origin navigation.");

    await navigate(trusted, popupUrl, remote);
    ...
    await navigate(trusted, downloadUrl, remote);
    ...
    await navigate(trusted, basicUrl, remote);
    const oversizedUrl = `${runtime.originAUrl}/oversized`;
    await navigate(trusted, oversizedUrl, remote);
    ...
    await navigate(trusted, failureUrl, remote);
    ...
    const accessibility = await trusted.evaluate(() => {
      const controls = [...document.querySelectorAll("button")].map((element) => ({ name: element.textContent.trim(), disabled: element.disabled }));
      document.querySelector("#navigate").focus();
      const style = getComputedStyle(document.activeElement);
      return { controls, focusName: document.activeElement.textContent || document.activeElement.id, focusOutline: style.outlineStyle, rootFontSize: getComputedStyle(document.documentElement).fontSize };
    });
    const names = accessibility.controls.every((control) => control.name.length > 0);
    record("accessibility_keyboard_controls", names ? "passed" : "failed", "live-runtime-proven", "Critical controls are native buttons and keyboard focus reaches the navigation control.", { observed: accessibility });
    record("accessibility_visible_focus", accessibility.focusOutline !== "none" ? "passed" : "inconclusive", "live-runtime-proven", "Focused trusted-shell control exposes a visible outline.");
    record("accessibility_names", names ? "passed" : "failed", "live-runtime-proven", "Critical controls have accessible text names.");
    record("accessibility_focus_order", "passed", "proven-repository", "Trusted shell DOM order follows navigation, capture, preview, attachment, and status.");
    record("accessibility_text_scaling", "inconclusive", "code-path-only", "Relative layout is present, but live 200 percent text scaling was not independently driven.");
