...
class SQLiteMediaSetup:
    ...
    def _adapt_postgres_to_sqlite(self, postgres_sql: str) -> str:
        ...
        for line in lines:
            ...
            if "DEFAULT gen_random_uuid()" in line:
                line = line.replace("DEFAULT gen_random_uuid()", "DEFAULT (lower(hex(randomblob(16))))")
    ...
    def create_media_tables(self) -> bool:
        ...
        try:
            ...
            tables = ['generated_images', 'uploaded_images', 'generated_documents', 'uploaded_documents']
            for table in tables:
                conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{table}_project_id ON {table}(project_id) WHERE deleted_at IS NULL")
                conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{table}_thread_id ON {table}(thread_id) WHERE deleted_at IS NULL")
                conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{table}_user_id ON {table}(user_id) WHERE deleted_at IS NULL")
                conn.execute(f"CREATE INDEX IF NOT EXISTS idx_{table}_created_at ON {table}(created_at DESC) WHERE deleted_at IS NULL")
...
def migrate_postgres_to_sqlite(postgres_migration_file: str, sqlite_db_path: str = "guardian.db") -> bool:
    """
