...
from typing import Tuple, Callable, Type, Any, Dict, AsyncGenerator
...
class _NoopLLMClient:
    def chat_completion(self, model=None, messages=None, temperature=0.7, max_tokens=1000):
...
settings = globals().get('settings') or SimpleNamespace(PROMPT_DIR_PATH=os.getenv('PROMPT_DIR_PATH', ''))
...
class ImprintZeroAgent:
    ...
    def __init__(self):
        ...
        try:
            ...
            mod = sys.modules.get('guardian.imprint_zero') or importlib.import_module('guardian.imprint_zero')
        ...
        dirs_to_check = [prompts_dir, prompts_dir / "prompts", prompts_dir / "imprint_zero"]
        ...
        if sys_text:
            ...
            logger.warning("No system prompt files found in %s; using fallback.", prompts_dir)
        ...
        if q_text:
            ...
            logger.warning("No question scaffold files found in %s; using fallback.", prompts_dir)
    ...
    def create_initial_profile(
        ...
        try:
            ...
            imprint_content = (
                f"Initial imprint for user {username}. Narrative Style: {narrative_style}."
            )
            memory.save(
