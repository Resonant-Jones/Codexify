...
import argparse
from guardian_rituals import (
    start_ritual_job,
    RITUAL_JOBS,
    seed_notion_db_with_progress,
    # In the future: import_from_drive_with_progress, etc.
)

def cli_seed_notion(args):
    from export_engine import load_records, get_notion_token
    ...
    print("Use 'ritual_cli.py ritual-status --job-id <id>' to check the progress of your magic.")
...
def cli_ritual_status(args):
    ...
    if not job:
        ...
        return
    print(f"Job {job.job_id}: {job.state} {job.current}/{job.total} ({job.description})")
    if job.state == "failed":
...
if __name__ == "__main__":
    ...
    parser_seed = subparsers.add_parser("seed-notion", help="Seed a Notion DB with progress tracking.")
    ...
    parser_status = subparsers.add_parser("ritual-status", help="Check status of a ritual job.")
