...
class CodexAwareness:
    ...
    def store_memory(
        ...
        logger.info(
            f"[TEMP_DEBUG] Storing memory_id: {artifact_id}, content: {content}, tags: {tags}"
        )  # DEBUG
        self.artifacts[artifact_id] = artifact
        self.save_artifacts()
        logger.info(
            f"[TEMP_DEBUG] CodexAwareness.artifacts after store for {artifact_id}: {list(self.artifacts.keys())}"
        )  # DEBUG
        return artifact_id
    ...
    def query_memory(
        ...
        """
        logger.info(
            f"[TEMP_DEBUG] query_memory: query='{query}', tags={tags}, current artifact keys: {list(self.artifacts.keys())}"
        )  # DEBUG
        results = []
        ...
        for (
            ...
            logger.info(
                f"[TEMP_DEBUG] Checking artifact: {artifact_id} with content type {artifact_obj.content.get('type', 'N/A')}"
            )  # DEBUG
            ...
            if tags and not all(tag in artifact_obj.tags for tag in tags):
                logger.info(
                    f"[TEMP_DEBUG] Artifact {artifact_id} failed tag check: artifact_tags={artifact_obj.tags}, query_tags={tags}"
                )  # DEBUG
                continue

            if artifact_obj.confidence < min_confidence:
                logger.info(
                    f"[TEMP_DEBUG] Artifact {artifact_id} failed confidence check: {artifact_obj.confidence} < {min_confidence}"
                )  # DEBUG
                continue
        ...
        logger.info(
            f"[TEMP_DEBUG] query_memory for '{query}' found {len(results)} results: {[r.id for r in results]}"
        )  # DEBUG
        return results
    ...
    def _matches_query(self, artifact: MemoryArtifact, query: str) -> bool:
        ...
        query_l = query.lower()
        logger.info(
            f"[TEMP_DEBUG] _matches_query: artifact ID {artifact.id}, query_l '{query_l}', artifact.content {artifact.content}, artifact.tags {artifact.tags}"
        )  # DEBUG
        ...
        if query in content_str:
            logger.info(
                f"[TEMP_DEBUG] _matches_query: Matched query '{query_l}' in content_str for artifact ID {artifact.id}"
            )  # DEBUG
            return True
        ...
        if artifact.tags and any(query_l in tag.lower() for tag in artifact.tags):
            logger.info(
                f"[TEMP_DEBUG] _matches_query: Matched query '{query_l}' in tags for artifact ID {artifact.id}"
            )  # DEBUG
            return True

        logger.info(
            f"[TEMP_DEBUG] _matches_query: No match for query '{query_l}' in artifact ID {artifact.id}"
        )  # DEBUG
        return False
