...
def verify_manifest_signature(manifest: dict, public_key_pem: bytes):
    ...
    try:
        ...
        public_key.verify(json.dumps(manifest_copy, sort_keys=True).encode(), signature)
        return True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Sign or verify Codexify federation manifests.")
    parser.add_argument("--node-id", required=True, help="Node identifier")
    parser.add_argument("--manifest", required=False, help="Path to manifest JSON")
    parser.add_argument("--verify", action="store_true", help="Verify a signed manifest")
    ...
    if args.verify:
        if not args.manifest:
            ...
            valid = verify_manifest_signature(manifest_data, pub_key.public_bytes(
                encoding=serialization.Encoding.PEM,
                format=serialization.PublicFormat.SubjectPublicKeyInfo
            ))
        ...
        # Sign manifest
        manifest_data = json.loads(Path(args.manifest).read_text()) if args.manifest else {
