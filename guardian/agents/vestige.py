...
class VestigeAgent:
        ...
        try:
            ...
            for memory in memories:
                if hasattr(memory, 'content'):
                    content_patterns = self._analyze_content_patterns(memory.content, None)
            ...
            if not verified_patterns:
                ...
                return [MemoryPattern(
                    ...
                    confidence=0.3,
                    artifacts=[str(m.id) for m in memories] if memories else ["unknown"],
                    metadata={
    ...
    def _analyze_temporal_patterns(
        ...
        for i in range(1, len(sorted_memories)):
            interval = (
                getattr(sorted_memories[i], 'timestamp', datetime.min) -
    ...
    def _calculate_confidence(
        ...
        verified_confidence = sum(
            p.confidence for p in patterns if p.verified
        )
