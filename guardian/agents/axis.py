...
class AxisAgent:
    ...
    def _validate_decision_input(
        ...
        if not all(
            field in context
            for field in required_fields[decision_type]
        ):
            return False
    ...
    async def record_outcome(
        ...
        try:
            ...
            if not decision_data:
                logger.warning(f"Decision {decision_id} not found, creating placeholder")
    ...
    def _strengthen_stability(self) -> None:
        ...
        for metric in self.stability_metrics:
            self.stability_metrics[metric] = min(
                1.0,
                self.stability_metrics[metric] * 0.9 + 0.1
            )
