...
from guardian.db import save_oauth_tokens, get_oauth_tokens  # implement per your DB layer
from guardian.crypto import encrypt_str, decrypt_str  # helpers (see below)

router = APIRouter(prefix="/connect/google", tags=["connectors"])

GOOGLE_CLIENT_ID = os.environ['GOOGLE_OAUTH_CLIENT_ID']
GOOGLE_CLIENT_SECRET = os.environ['GOOGLE_OAUTH_CLIENT_SECRET']
REDIRECT_URI = os.environ.get('GOOGLE_OAUTH_REDIRECT', 'http://localhost:8000/connect/google/callback')
...
def callback(code: str = None, state: str = None, request: Request = None):
    ...
    if "expires_in" in token_data:
        expires_at = (int(time.time()) + int(token_data["expires_in"]))  # store as epoch or ISO
