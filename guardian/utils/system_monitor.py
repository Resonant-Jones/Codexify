...
class SystemMonitor:
    ...
    async def enforce_limits(self, usage: ResourceUsage) -> None:
        ...
        if usage.disk_usage_percent > self.limits['disk_percent']:
            warnings.append(
                f"High disk usage: {usage.disk_usage_percent:.1f}%"
            )
        
        # Check open files
        if usage.open_files > self.limits['max_files']:
            warnings.append(
                f"Too many open files: {usage.open_files}"
            )
        
        # Check thread count
        if usage.thread_count > self.limits['max_threads']:
            warnings.append(
                f"Too many threads: {usage.thread_count}"
            )
    ...
    async def _handle_resource_pressure(self, usage: ResourceUsage) -> None:
        ...
        await system_logger.error(
            "Resource limits exceeded - taking action",
            usage=vars(usage)
        )
        ...
        for plugin in plugin_manager.plugins.values():
            if (
                plugin.enabled and
                'essential' not in plugin.metadata.get('tags', [])
            ):
