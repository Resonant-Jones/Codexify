...
print(
    "✅ conftest.py(seeding):",
    "GENAI_API_KEY=", bool(os.environ.get("GENAI_API_KEY")),
...
try:
    ...
    def _patched_get_settings(*args, **kwargs):
        ...
        for _k in [
            "GENAI_API_KEY", "NOTION_API_KEY", "ANTHROPIC_API_KEY",
...
def pytest_collection_modifyitems(config, items):
    ...
    net_skip = pytest.mark.skip(reason="requires network (set ALLOW_NET_TESTS=1 to enable)")
    for item in items:
...
def _seed_dummy_env_for_settings():
    ...
    """
    _ = bool(os.getenv("ALLOW_NET_TESTS"))  # not used currently; keep for future behavior
...
def _force_dummy_llm(monkeypatch, dummy_client):
    ...
    try:
        ...
        if hasattr(mem, "_build_llm_client_from_env"):
            monkeypatch.setattr(mem, "_build_llm_client_from_env", lambda: dummy_client, raising=False)
    except Exception:
        pass
