...
def _valid_invite_for_resolution(
    ...
    row = session.scalar(
        select(AccountObservabilityInviteLink).where(
            AccountObservabilityInviteLink.token_hash
            == hash_invite_token(token)
        )
    )
    if (
        row is None
        or row.status != AccountObservabilityInviteStatus.ACTIVE.value
    ):
        return None
    ...
    *,
    ...
    if invite is None:
        return InviteResolutionResult(
            result=None, guest_id=None, invite_id=None
        )
...
def is_eligible_human_registration(user: User) -> bool:
    ...
    return not any(
        candidate in _NON_HUMAN_IDS or candidate.startswith(_NON_HUMAN_PREFIXES)
...
def complete_registration_attribution(
    ...
    *,
    ...
    if (
        guest is not None
        and invite_id is not None
        and guest.converted_at is None
    ):
...
def record_invite_audit(
    ...
    *,
    ...
    if action not in {
        item.value for item in AccountObservabilityInviteAuditAction
    }:
