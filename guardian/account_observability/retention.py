    ...
    while True:
        if dry_run:
            batch = session.execute(
                select(AccountObservabilityPresenceSession.presence_session_id)
                .where(
                    AccountObservabilityPresenceSession.created_at
                    < presence_cutoff
                )
...
def run_cleanup(
    ...
    *,
    ...
    expired = _expire_idle_sessions(
        session, idle_cutoff, current, dry_run=dry_run
    )
