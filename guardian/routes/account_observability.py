...
def configure_db(db: Any) -> None:
    """Bind the application GuardianDB using the existing route convention."""
...
def _db_or_503() -> Any:
    db = _db or load_guardian_db_from_env()
    if db is None:
...
def create_operator_invite(
    ...
    try:
        ...
        raise HTTPException(
            status_code=409, detail="Invite creation conflict"
        ) from exc
...
def _transition_invite(
    ...
    *,
    ...
    try:
        ...
        raise HTTPException(
            status_code=409, detail="Invalid invite transition"
        ) from exc
...
def _resolve_subject(
    ...
    *,
    ...
    """Resolve one account or server-issued guest from trusted auth context."""

    guest_id = str(guest_cookie or "").strip() or None
    ...
    if x_api_key:
        ...
        if not account_id:
            raise HTTPException(
                status_code=401, detail="Authenticated account required"
            )
        if guest_id is not None:
            raise HTTPException(
                status_code=422, detail="Ambiguous presence subject"
