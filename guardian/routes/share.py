...
class CreateShareRequest(BaseModel):
    ...
    target_id: int
    expires_in_days: Optional[int] = None  # Optional expiry in days
    ...
    try:
        ...
        with db.get_session() as session:
            ...
            if request.target_type == 'thread':
                target = session.query(models.ChatThread).filter_by(id=request.target_id).first()
                if not target:
                    logger.warning(f"Thread {request.target_id} not found for sharing")
                    raise HTTPException(
                ...
                target = session.query(models.GeneratedDocument).filter_by(id=str(request.target_id)).first()
                if not target:
                    target = session.query(models.UploadedDocument).filter_by(id=str(request.target_id)).first()

                if not target:
                    logger.warning(f"Document {request.target_id} not found for sharing")
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
            ...
            logger.info(f"Created share link {share_id} for {request.target_type} {request.target_id}")
...
async def retrieve_share_content(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with db.get_session() as session:
            ...
            shared_link = session.query(models.SharedLink).filter_by(token=token).first()
            ...
            # Check expiry
            if shared_link.expires_at and datetime.now(UTC) > shared_link.expires_at:
            ...
            if shared_link.target_type == 'thread':
                thread = session.query(models.ChatThread).filter_by(id=shared_link.target_id).first()

                if not thread:
                    logger.warning(f"Thread {shared_link.target_id} referenced by share token not found")
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
                ...
                messages = session.query(models.ChatMessage).filter_by(
                    thread_id=thread.id
                ...
                document = session.query(models.GeneratedDocument).filter_by(
                    id=str(shared_link.target_id)
                ).first()

                if not document:
                    # Try UploadedDocument
                    document = session.query(models.UploadedDocument).filter_by(
                        id=str(shared_link.target_id)
                    ).first()

                if not document:
                    logger.warning(f"Document {shared_link.target_id} referenced by share token not found")
                    raise HTTPException(
                        status_code=status.HTTP_404_NOT_FOUND,
