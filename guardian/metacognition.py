...
class MetacognitionEngine:
    ...
    def system_health_check(self) -> Dict[str, Any]:
        ...
        thread_health = (
            self.thread_manager.health_check() if self.thread_manager 
    ...
    def _assess_overall_health(
        ...
        if (memory_status == "error" or
            "error" in agent_status.values() or
            thread_health.get('status') == "error"):
            return "error"
        
        if (memory_status == "warning" or
            "warning" in agent_status.values() or
            thread_health.get('status') == "warning"):
            return "warning"
    ...
    def _assess_confidence(
        ...
        if confidence_score < 0.7:
            ...
            if not required_agents.issubset(set(active_agents)):
                recommendations.append(
                    "Ensure all required agents are active"
                )
    ...
    def _identify_affected_components(
        ...
        return affected or ['system']  # Default to system-wide if no specific components
    ...
    async def check_recovery_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recovery_memories:
                ...
                if (current_time - recovery_time).total_seconds() < 300:  # Within 5 minutes
                    return {
                        'status': 'recovered',
