...
def _has_column(inspector, table: str, column: str) -> bool:
    return column in {col["name"] for col in inspector.get_columns(table, schema="public")}
...
def upgrade() -> None:
    ...
    if not _has_table(inspector, "connector_configs"):
        ...
        op.add_column("connector_configs", sa.Column("schedule", sa.String(length=255), nullable=True))

    if not _has_table(inspector, "connector_runs"):
        op.create_table(
            "connector_runs",
            sa.Column("id", sa.BigInteger(), primary_key=True, autoincrement=True),
            sa.Column("config_id", sa.Integer(), sa.ForeignKey("connector_configs.id", ondelete="CASCADE"), nullable=False),
        ...
        op.create_index("ix_connector_runs_config_started", "connector_runs", ["config_id", sa.literal_column("started_at DESC")])
    ...
    if not _has_table(inspector, "raw_documents"):
        ...
        op.create_index("ix_raw_documents_config_external", "raw_documents", ["config_id", "external_id"], unique=True)
    ...
    if not _has_table(inspector, "events_outbox"):
        ...
        op.create_index("ix_events_outbox_status_created", "events_outbox", ["status", "created_at"])
        op.create_index("ix_events_outbox_tenant_id", "events_outbox", ["tenant_id"])
    ...
    if not _has_table(inspector, "memory_entries"):
        op.create_table(
            ...
            sa.Column("created_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()"), nullable=False),
            sa.Column("updated_at", sa.TIMESTAMP(timezone=True), server_default=sa.text("now()"), nullable=False),
            sa.CheckConstraint("silo IN ('ephemeral', 'midterm', 'longterm')", name="memory_entries_silo_check"),
        )
        op.create_index("ix_memory_entries_silo", "memory_entries", ["silo"])
        op.create_index("ix_memory_entries_user_silo", "memory_entries", ["user_id", "silo"])
        op.create_index("ix_memory_entries_silo_updated", "memory_entries", ["silo", "updated_at"])
    ...
    if not _has_table(inspector, "chat_threads"):
        ...
        op.create_index("ix_chat_threads_parent_id", "chat_threads", ["parent_id"])
        op.create_index("ix_chat_threads_updated", "chat_threads", [sa.literal_column("updated_at DESC")])
