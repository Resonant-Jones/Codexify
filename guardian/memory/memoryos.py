...
class MemoryOS:
    ...
    def monitor_conversation_length(self, conversation_id: str) -> Dict[str, Union[str, bool]]:
        """
    ...
    def summarize_conversation(self, conversation_id: str) -> Dict[str, str]:
        ...
        try:
            ...
            if not child:
                return {"status": "error", "message": "Failed to create child conversation"}
                
            # Save updated parent
            self.conversation_manager.save_conversation(conversation)
            
            logger.info(f"Summarized conversation {conversation_id}, created child {child.id}")
