...
export const createProject = (title: string) =>
  apiRequest<{ project_id: number }>("api/projects", {
    method: "POST",
...
export const createThread = (projectId: number, title: string) =>
  apiRequest<{ thread_id: number }>("api/chat/threads", {
    method: "POST",
    body: JSON.stringify({ project_id: projectId, title, summary: "" }),
  });

export const deleteThread = (id: string) =>
  apiRequest<void>(`api/chat/threads/${id}`, { method: "DELETE" });

export const listThreads = () =>
  apiRequest<{ threads: any[] }>("api/chat/threads", { method: "GET" });

// Chat
// Chat: backend expects { prompt, model? } and returns { reply, model_used }
export const sendChat = (threadId: string, content: string) =>
  apiRequest<{ reply: string; model_used: string }>("api/chat", {
    method: "POST",
