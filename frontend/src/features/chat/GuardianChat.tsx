...
export function GuardianChat({
  ...
  const composerProjectName = (() => {
    const selectedProject = projectOptions.find(
      (project) => String(project.id) === String(composerProjectId)
    );
    if (workspaceProjectId != null) {
      return workspaceProjectName ?? selectedProject?.name ?? null;
    }
    return activeThread.projectName ?? selectedProject?.name ?? null;
  })();
