...
_REQUIRED_SCHEDULE_KEYS = [
    "id", "label", "cadence", "enabled", "activation_mode",
    "run_command", "review_after", "review_command",
    "required_inputs", "generated_artifacts", "publication",
]
...
ALLOWED_ACTIVATION_MODES = {"manual_only", "local_scheduler_ready", "future_codexify_cron_ready"}


def test_activation_mode_is_allowed() -> None:
    d = _load_manifest()
    for s in d["schedules"]:
        assert s["activation_mode"] in ALLOWED_ACTIVATION_MODES, (
...
def test_run_command_points_to_make_heartbeat() -> None:
    d = _load_manifest()
    for s in d["schedules"]:
        assert "make heartbeat" in s["run_command"], (
...
def test_review_command_points_to_make_heartbeat_review() -> None:
    d = _load_manifest()
    for s in d["schedules"]:
        assert "make heartbeat-review" in s["review_command"], (
