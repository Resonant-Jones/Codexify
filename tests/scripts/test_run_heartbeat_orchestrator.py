...
def _make_successful_run(args: list[str], **kw: object) -> subprocess.CompletedProcess[str]:
    """Return a successful CompletedProcess based on which script is called."""
    cmd_str = " ".join(str(a) for a in args)
    if "beta_release_sentinel" in cmd_str:
        return subprocess.CompletedProcess(args=args, returncode=0, stdout=_SUCCESSFUL_SENTINEL_OUTPUT, stderr="")
    elif "ingest_daily_dev_blog" in cmd_str:
        return subprocess.CompletedProcess(args=args, returncode=0, stdout=_SUCCESSFUL_DEV_BLOG_OUTPUT, stderr="")
    elif "generate_resonant_daily_insight" in cmd_str:
        return subprocess.CompletedProcess(args=args, returncode=0, stdout=_SUCCESSFUL_INSIGHT_OUTPUT, stderr="")
        ...
        if "show-current" in cmd_str:
            ...
            return subprocess.CompletedProcess(args=args, returncode=0, stdout="abc123def\n", stderr="")
...
def test_dry_run_prints_planned_commands_and_writes_nothing(tmp_path: Path) -> None:
    ...
    result = _run_cli(
        "--date", "2026-05-13",
        "--dev-blog-source", str(src),
        "--insight-source", str(insight_src),
...
def test_successful_orchestration_with_mocked_subprocesses(tmp_path: Path) -> None:
    output_dir = tmp_path / "heartbeat-out"
    src = _make_md_file(tmp_path, "blog.md", "# Blog\n\nBody.\n")
    insight_src = _make_md_file(tmp_path, "insight.md", "# Insight\n\nBody.\n")
...
def test_report_includes_artifact_paths(tmp_path: Path) -> None:
    ...
    assert "docs/Website/dev-blog/generated" in content or "target_path" in content
...
def test_existing_report_without_force_fails(tmp_path: Path) -> None:
    ...
    assert result.returncode != 0
    assert "already exists" in result.stderr.lower() or result.stderr.lower().find("already exists") >= 0
...
def _make_failing_dev_blog(args: list[str], **kw: object) -> subprocess.CompletedProcess[str]:
    cmd_str = " ".join(str(a) for a in args)
...
def _make_secret_leaking_run(args: list[str], **kw: object) -> subprocess.CompletedProcess[str]:
    cmd_str = " ".join(str(a) for a in args)
    if "ingest_daily_dev_blog" in cmd_str:
        ...
        return subprocess.CompletedProcess(
            args=args, returncode=1,
            stdout="",
            stderr="error: api_key=sk-abcdefghijklmnopqrstuvwxyz123456 not valid",
        )
    if "beta_release_sentinel" in cmd_str:
        ...
        return subprocess.CompletedProcess(
            ...
            stderr="access_token=ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890",
        )
    if "generate_resonant_daily_insight" in cmd_str:
        return subprocess.CompletedProcess(
            args=args, returncode=1,
...
def test_report_sanitizes_secret_like_values(tmp_path: Path) -> None:
    ...
    assert "ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890" not in content
