...
def test_successful_staging(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(stage_mod, "REVIEW_SCRIPT", ROOT / "scripts" / "content" / "review_heartbeat_run.py")
    ...
    def _mock_review_run(args, **kw):
        if "review_heartbeat_run" in str(args):
            return subprocess.CompletedProcess(
                ...
                stdout=json.dumps({"status": "passed", "issues": [], "warnings": []}),
                stderr="",
            )
...
def test_review_failure_blocks_staging(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    heartbeat_dir = _setup_heartbeat_run(tmp_path, "2026-05-14", fail_review=True)
    staged_dir = tmp_path / "staged"
    monkeypatch.setattr(stage_mod, "REPO_ROOT", tmp_path)
    monkeypatch.setattr(stage_mod, "REVIEW_SCRIPT", ROOT / "scripts" / "content" / "review_heartbeat_run.py")
    ...
    def _mock_review_run(args, **kw):
        if "review_heartbeat_run" in str(args):
            return subprocess.CompletedProcess(
                ...
                stdout=json.dumps({"status": "warning", "issues": ["failed steps"], "warnings": []}),
                stderr="",
            )
        return subprocess.CompletedProcess(args=args, returncode=0, stdout="", stderr="")
...
def test_skip_review_bypasses_gate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    heartbeat_dir = _setup_heartbeat_run(tmp_path, "2026-05-14", fail_review=True)
    staged_dir = tmp_path / "staged"
    monkeypatch.setattr(stage_mod, "REPO_ROOT", tmp_path)
...
def test_force_overwrites_existing_staged(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    heartbeat_dir = _setup_heartbeat_run(tmp_path, "2026-05-14")
    staged_dir = tmp_path / "staged"
    staged_dir.mkdir(parents=True)
    # Pre-create a file that would collide
    (staged_dir / "2026-05-14").mkdir(parents=True, exist_ok=True)
    (staged_dir / "2026-05-14" / "2026-05-14-beta-sentinel.md").write_text("old", encoding="utf-8")
    ...
    content = (staged_dir / "2026-05-14" / "2026-05-14-beta-sentinel.md").read_text(encoding="utf-8")
...
def test_without_force_nonempty_staged_fails(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Without --force, non-empty staged directory is a hard failure."""
    heartbeat_dir = _setup_heartbeat_run(tmp_path, "2026-05-14")
    staged_dir = tmp_path / "staged"
    staged_dir.mkdir(parents=True)
    (staged_dir / "2026-05-14").mkdir(parents=True, exist_ok=True)
    (staged_dir / "2026-05-14" / "old-file.md").write_text("old", encoding="utf-8")
