...
def _setup_staged_dir(tmp_path: Path, date_str: str, *, review_skipped: bool = False) -> Path:
    ...
    (staged_dir / "release-summary.md").write_text("# Summary", encoding="utf-8")

    if review_skipped:
        (staged_dir / "_SKIP_REVIEW_WARNING.txt").write_text("SKIPPED", encoding="utf-8")
