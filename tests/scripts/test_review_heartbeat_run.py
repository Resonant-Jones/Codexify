...
def test_valid_report_passes(tmp_path: Path) -> None:
    ...
    result = _run_cli("--date", "2026-05-13", "--heartbeat-dir", str(tmp_path), "--json")
...
def test_missing_report_is_warning(tmp_path: Path) -> None:
    ...
    assert "NOT FOUND" in result.stdout or "no heartbeat report" in result.stdout.lower()
...
def test_title_date_mismatch_is_warning(tmp_path: Path) -> None:
    ...
    result = _run_cli("--date", "2026-05-14", "--heartbeat-dir", str(tmp_path), "--json")
    d = json.loads(result.stdout)
    assert d["status"] == "warning"
