...
def test_imprint_activation_supersedes_prior():
    ...
    im1 = imprint_store.save_imprint("u1", None, guardian_name="G1", status="draft")
    ...
    im2 = imprint_store.save_imprint("u1", None, guardian_name="G2", status="draft")
...
def test_system_docs_links_and_globals():
    ...
    with Session() as session:
        global_doc = SystemDoc(scope="global", slug="g1", title="Global Doc", content="global", is_enabled=True)
        linked_doc = SystemDoc(scope="user", owner_user_id="u1", slug="l1", title="Linked", content="linked", is_enabled=True)
        disabled_doc = SystemDoc(scope="user", owner_user_id="u1", slug="d1", title="Disabled", content="nope", is_enabled=False)
        session.add_all([global_doc, linked_doc, disabled_doc])
        session.commit()

        link = SystemDocLink(user_id="u1", project_id=None, system_doc_id=linked_doc.id, is_enabled=True)
        session.add(link)
        session.commit()
