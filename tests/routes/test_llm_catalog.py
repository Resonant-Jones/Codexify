...
def _mock_whooshd_mixed_catalog_request(
    url: str, *args, **kwargs
) -> _MockResponse:
    if url == "http://host.docker.internal:8000/api/tags":
        return _MockResponse(
...
def test_llm_catalog_merges_whooshd_inventory_sources(
    monkeypatch,
):
    monkeypatch.setattr(
        llm_catalog.requests, "get", _mock_whooshd_mixed_catalog_request
    )
    _clear_extra_cloud_keys(monkeypatch)
    ...
    try:
        ...
        assert "gemma-4-e4b-it-4bit" in model_ids
        assert "gemma-4-12b-it-qat-4bit" in model_ids
        assert local["default_model"] == "mlx-community/gemma-4-e4b-it-4bit"
        assert local["enabled"] is True
        assert local["truth"]["selectable"] is True
        assert local["endpoint_resolution"]["inventory_endpoint"] == "/v1/models"
    finally:
        for field, value in snapshot.items():
            setattr(settings, field, value)
