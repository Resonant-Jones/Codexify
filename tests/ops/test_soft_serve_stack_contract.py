...
ADR_PATH = (
    ROOT
    / "docs/architecture/adr/054-vaultnode-soft-serve-local-forge-and-github-publication-remote.md"
)
...
def test_soft_serve_compose_isolated_and_pinned() -> None:
    ...
    assert re.findall(r"^  [a-zA-Z0-9_-]+:$", text, re.MULTILINE) == [
        "  soft-serve:"
    ]
    ...
    assert (
        "SOFT_SERVE_INITIAL_ADMIN_KEYS is required for Soft Serve startup"
        in text
    )
...
def test_soft_serve_operator_script_exposes_exact_commands_and_is_executable() -> (
    None
):
