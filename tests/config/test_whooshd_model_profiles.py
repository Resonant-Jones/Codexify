...
E4B_OPTIQ_PROFILE_PATH = PROFILE_DIR / "gemma-4-e4b-it-optiq-4bit.json"
...
def _load_e4b_optiq_profile() -> dict:
    return json.loads(E4B_OPTIQ_PROFILE_PATH.read_text(encoding="utf-8"))
...
def test_validator_passes_for_committed_profiles() -> None:
    ...
    assert "Validated 5 Whoosh'd model profile(s)." in result.stdout
...
def test_gemma_4_e4b_optiq_profile_preserves_local_provider_boundary() -> None:
    profile = _load_e4b_optiq_profile()

    assert profile["id"] == "gemma-4-e4b-it-optiq-4bit"
    assert profile["provider_id"] == "local"
    assert profile["runtime"]["server"] == "mlx_vlm.server"
