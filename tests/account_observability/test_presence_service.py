...
def _seed_guest(session: Session, guest_id: str, invite_id: str | None = None) -> None:
...
class TestConcurrentHeartbeats:
    def test_concurrent_heartbeats_preserve_one_coherent_state(self, db_session):
        ...
        sessions = db_session.execute(
            select(AccountObservabilityPresenceSession).where(
                AccountObservabilityPresenceSession.user_id == "user-concurrent",
...
class TestServerTimeControlsLease:
    ...
    def test_ignores_client_timestamps(self, db_session):
        ...
        assert abs((row.started_at.replace(tzinfo=timezone.utc) - now).total_seconds()) < 1
