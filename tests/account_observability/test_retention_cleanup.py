...
def _seed_account_metadata(
    session, user_id: str, prior_guest_id: str | None = None
) -> None:
...
class TestIdleSessionExpiry:
    def test_idle_sessions_are_ended(self, db_session):
        ...
        _seed_presence(
            db_session, "p-old", "user-idle", None, old_active, old_active
        )
        _seed_presence(
            db_session,
            "p-recent",
    ...
    def test_recent_sessions_are_untouched(self, db_session):
        ...
        _seed_presence(
            db_session, "p-recent", "user-recent", None, recent, recent
        )
...
class TestGuestLineageCleanup:
    ...
    def test_converted_guest_with_metadata_is_deferred(self, db_session):
        ...
        _seed_guest(
            db_session, "guest-converted", old_created, converted_at=old_created
        )
        _seed_account_metadata(
            db_session, "user-guest", prior_guest_id="guest-converted"
        )
        ...
        guest = db_session.get(
            AccountObservabilityGuestIdentity, "guest-converted"
        )
...
class TestCleanupReceipt:
    def test_receipt_has_accurate_cutoffs(self, db_session):
        ...
        assert (
            abs(
                (
                    receipt.cutoff_presence_30d - expected_presence
                ).total_seconds()
            )
            < 1
        )
    ...
    def test_dry_run_does_not_mutate(self, db_session):
        ...
        _seed_presence(
            db_session, "p-dry", "user-dry", None, old_time, old_time
        )
