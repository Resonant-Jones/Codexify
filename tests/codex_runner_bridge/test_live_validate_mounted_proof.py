...
def test_mounted_proof_has_status() -> None:
    ...
    statuses = ["Status: PASS", "Status: FAIL", "Status: BLOCKED",
                "Status: **FAIL**", "Status: **PASS**", "Status: **BLOCKED**"]
...
def test_mounted_proof_mentions_compose_override() -> None:
    ...
    assert "compose" in text and ("override" in text or "codex-runner-bridge" in text), (
...
def test_mounted_proof_states_no_orchestration_proof() -> None:
    text = MOUNTED_PROOF.read_text().lower()
    assert "no live orchestration" in text or "not live orchestration" in text or "does not prove live orchestration" in text, (
...
def test_mounted_proof_states_no_ui_support() -> None:
    text = MOUNTED_PROOF.read_text().lower()
    assert "no ui support" in text or "does not prove guardian ui" in text or "no ui" in text, (
...
def test_mounted_proof_states_no_codexify_ingestion() -> None:
    text = MOUNTED_PROOF.read_text().lower()
    assert "no codexify ingestion" in text or "does not ingest" in text or "does not ingest evidence" in text, (
...
def test_visibility_contract_links_mounted_proof() -> None:
    ...
    assert "guardian-codex-runner-command-bus-live-validate-mounted-proof.md" in text, (
