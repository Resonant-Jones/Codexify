...
def test_parse_args_pi_flags_work(tmp_path: Path) -> None:
    ...
    assert args.provider == "pi"
    assert args.pi_model == "sonnet"
    assert args.pi_provider == "anthropic"
    assert args.pi_thinking == "medium"
    assert args.execute is False
    assert args.dry_run is True


def test_parse_args_legacy_provider_fails_closed(tmp_path: Path) -> None:
    audit_prompt = tmp_path / "audit.md"
    audit_schema = tmp_path / "audit.schema.json"
    compiler_prompt = tmp_path / "compiler.md"
    campaign_schema = tmp_path / "campaign.schema.json"
