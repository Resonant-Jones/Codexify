...
/src
├── components/
│   ├── ui/                       // Replicas of shadcn/ui components
│   │   ├── Avatar.js
│   │   ├── Badge.js
│   │   ├── Button.js
│   │   ├── Card.js
...
Start by creating the folder structure above in your project's src directory.

2. Move Helper Functions

Create a file named themeUtils.js inside the utils directory.

Move the hexToRgb, rgbToHsl, hslToHex, rotateHue, darken, lighten, relativeLuminance, and contrastRatioHex functions into this file.

At the top of themeUtils.js, add export to each function you want to make available. For example: export function hexToRgb(hex) { ... }.

3. Move Replicated UI Components

Create a file for each of the replicated shadcn/ui components (e.g., Button.js, Card.js, Input.js, etc.) in the components/ui directory.

Copy the corresponding component code into its new file.

Add export const or export default to each component definition. For instance: export const Button = React.forwardRef(...) or export default Button;.

4. Extract Core Components

ChatBubble.js: Create this file in components/chat and move the ChatBubble function into it. It will need to import motion from framer-motion and fmtTime from your utility functions.

Composer.js: Create in components/chat, move the Composer function. It will need to import useState, useRef, useEffect, Button, Textarea, and Send (from lucide-react).

Sidebar.js: Create in components/chat, move Sidebar. It will need useMemo, useState, Search, Plus, Input, Avatar, AvatarFallback, AvatarImage, and Badge.

DashboardView.js: Create in components/dashboard, move DashboardView. It will need Card, CardContent, Button, DocumentTile, and ext.

SettingsView.js: Create in components/settings, move SettingsView. It will need useState, useEffect, useRef, Card, CardContent, Button, Input, Textarea, ImagePlus, FolderOpen, SegmentedThemeControl, and ContrastChip.

WorkspacePane.js: Create in components/layout, move WorkspacePane. It will need Card, FileText, and ext.

5. Create the useThemeController Hook

Create useThemeController.js in the hooks directory.

Move the coerceMode and useThemeController functions into this file.

Export both functions: export function coerceMode(v) { ... } and export function useThemeController() { ... }.

6. Update the Main App.js File

The main App.js file should now be much cleaner.

Import all the top-level components and hooks you just created.

The App component will remain, but with significantly fewer lines of code.

Here is a simplified example of what your App.js would look like after the refactor:

JavaScript
import React, { useEffect, useState } from "react";
import { Separator } from "./components/ui/Separator";
import { Button } from "./components/ui/Button";
import { GuardianChat } from "./components/chat/GuardianChat";
import { DashboardView } from "./components/dashboard/DashboardView";
import { SettingsView } from "./components/settings/SettingsView";
import { WorkspacePane } from "./components/layout/WorkspacePane";
import { useThemeController } from "./hooks/useThemeController";
import { rotateHue, darken, lighten } from "./utils/themeUtils";
...
This modular approach makes the codebase easier to read, debug, and maintain. If you want to change the chat bubble, you only need to look in ChatBubble.js and its related files, rather than searching through a massive single file.
...
// The following are simplified, self-contained replicas of shadcn/ui components
// to make the application fully runnable without external dependencies.
// -------------------------------------------------------------------------
...
const DropdownMenuTrigger = React.forwardRef(({ children, asChild, ...props }, ref) => {
  ...
  return <button {...newProps}>{child}</button>;
});
...
const Sheet = ({ children, open, onOpenChange }) => {
  ...
  return React.Children.map(children, child => {
    ...
    if (child.type.displayName === "SheetContent" && (isOpen || open)) {
      return React.cloneElement(child, { handleClose: () => handleOpenChange(false) });
    }
    return null;
  });
};
...
const SheetContent = ({ children, side, handleClose, ...props }) => {
  const sideClass = side === "left" ? "left-0" : "right-0";
  return (
    <div className={`fixed inset-y-0 z-50 w-full md:w-80 border-r bg-background shadow-xl transition-transform duration-300 ease-in-out transform translate-x-0 ${sideClass}`} {...props}>
      <button onClick={handleClose} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700">&times;</button>
      {children}
    </div>
  );
};
...
const SheetHeader = ({ children, className = "" }) => <div className={`flex flex-col space-y-2 text-center sm:text-left ${className}`}>{children}</div>;
const SheetTitle = ({ children, className = "" }) => <div className={`text-lg font-semibold text-foreground ${className}`}>{children}</div>;
...
const VisuallyHidden = ({ children }) => <span style={{ position: 'absolute', width: '1px', height: '1px', padding: '0', margin: '-1px', overflow: 'hidden', clip: 'rect(0, 0, 0, 0)', whiteSpace: 'nowrap', borderWidth: '0' }}>{children}</span>;
